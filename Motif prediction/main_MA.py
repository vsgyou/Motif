...
end_date = '2023-07-30'
samsung_data = yf.download(stock_code, 
                           start = start_date, 
                           end = end_date)
close = samsung_data['Close']
close_MA = MA(close, p=5)
...
#%%
train_set, test_set = split_data(data = close_MA, input_window = 7, test_len = 60)
train_set, valid_set = split_data(data = train_set, input_window = 7, test_len = 60)

train_set = windowDataset(data = train_set, 
                          input_window = 7, 
                          output_window = 1, 
                          input_size = 1, 
                          stride = 1)
valid_set = windowDataset(data = valid_set, 
                          input_window = 7, 
                          output_window = 1, 
                          input_size = 1, 
                          stride = 1)
test_set = windowDataset(data = test_set, 
                         input_window = 7, 
                         output_window = 1, 
                         input_size = 1, 
                         stride = 1)
close_set = windowDataset(data = close_MA, 
                          input_window = 7, 
                          output_window = 1, 
                          input_size = 1, 
                          stride = 1)
...
train_pattern = torch.zeros([train_trans.x.shape[0],
                             train_trans.x.shape[1],k])
train_pattern_noscale = torch.zeros([train_set_org.x.shape[0],
                                     train_set_org.x.shape[1],k]) 
valid_pattern = torch.zeros([valid_trans.x.shape[0],
                             valid_trans.x.shape[1],k])
valid_pattern_noscale = torch.zeros([valid_set_org.x.shape[0],
                                     valid_set_org.x.shape[1],k]) 
test_pattern = torch.zeros([test_trans.x.shape[0],
                            test_trans.x.shape[1],k])
test_pattern_noscale = torch.zeros([test_set_org.x.shape[0],
                                    test_set_org.x.shape[1],k]) 
...
test_acc_MA = calculate_accuracy(test_predictions, close[-60:])
